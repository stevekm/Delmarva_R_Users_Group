...
\begin{abstract}
A data set was acquired from academic sources, describing qualities of an industrial adhesive compound under differing conditions of temperature and pressure. Adhesive strength was measured, and statistical analysis will be used to determine if significant differences exist between different conditions.
Document updated on \date{\today}.
\end{abstract}

\section{Data Entry}
We will be analyzing data describing the strength of an industrial adhesive compound under varying levels of temerature and pressure. The data was obtained from academic sources, and is entered in to R in the following code chunk.
<<StartHere>>=
...
@
The normality tests  all showed p values greater than 0.05, suggesting that the data is normally distributed. 
...
lines(x  =Pi,#plots a normal curve based on the data
      ...
      col='gray25')
@
In this section, we generated a Q-Q plot in order to show the distribution of the data to again check for normality, along with a histogram overlaying an ideal normal distribution curve over the data. Again, the data seems to have a roughly normal distribution. 
\section{Homogeneity}
In this section, we will test the data for homogeneity of variance. The null hypothesis assumes homogeneity.
<<bartletts_test>>=
bartlett.test(formula=Readings~Temp,data=Data)
bartlett.test(formula=Readings~Pressure,data=Data)
@
P values in these tests were both above 0.05, suggesting that the variance in each group is the same.
...
@ 
Tukey tests were used to test for homogeneity of subsets, and for non-additivity. The HSD test showed that Temperature levels 260 and 250 group together with a P value of 0.05, while levels 260 and 270 group together with a P value of 0.04. No other temperature or Pressure groups showed homogeneity. The 95 percent confidence intervals were also calculated. 

\section{Conclusion}
In conclusion, the results suggest that there is a difference in adhesive strength under different levels of Temperature (p less than 0.05), causing us to reject the null hypothesis. The 260 \& 270, and 250\& 260 Temperature groups were suggested to be homogeneous subsets. Data showed a normal distribution. 
